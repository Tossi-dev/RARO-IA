-- ============================================================
-- Raro.ia — Expansão v2 (Plano de Expansão aprovado 2026-07-16)
-- Rodar DEPOIS do 0001_schema.sql, no SQL Editor do Supabase.
-- Adiciona: CRM por estágios, notas/atividades/tarefas, reuniões +
-- transcrições, orçamentos/metas, redes sociais (perfis, conteúdos,
-- métricas, retenção, pilares) e campanhas.
-- ============================================================
...
-- ---------- CRM: estágios customizáveis ----------
create table public.crm_estagios (
  id uuid primary key default gen_random_uuid(),
  nome text not null,
  ordem int not null default 0,
  cor text not null default 'cinza',            -- tom do badge na UI
  funil status_funil not null default 'potencial', -- mapeia p/ métricas do funil
...
create table public.transcricoes (
  ...
  origem text not null default 'manual',        -- manual | audio_ia
...
create table public.metas_financeiras (
  id uuid primary key default gen_random_uuid(),
  tipo text not null,                           -- faturamento | lucro
  periodo text not null,                        -- 'YYYY-MM'
...
create table public.perfis_sociais (
  ...
  conectado boolean not null default false,     -- true quando token real configurado
...
create table public.conteudos (
  ...
  externo_id text not null default ''           -- id na plataforma (sync API)
);
...
create table public.conteudo_retencao (
  id uuid primary key default gen_random_uuid(),
  conteudo_id uuid not null references public.conteudos (id) on delete cascade,
  ponto_pct int not null,                       -- 0..100 (posição no vídeo)
  retencao_pct numeric(5,2) not null            -- % da audiência que continua
);
...
create table public.conteudo_pilares (
  ...
  texto text not null default '',
  nota numeric(3,1),                            -- 0..10 (avaliação manual)
...
create table public.campanhas (
  ...
  canal text not null default 'multi',          -- instagram | tiktok | facebook | multi

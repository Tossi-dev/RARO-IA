-- ============================================================
-- Raro.ia — Schema inicial (Prioridade 1)
-- Rodar no SQL Editor do Supabase (uma vez, projeto novo).
-- Modelo: single-tenant (negócio do Jefson). Se um dia virar
-- multi-tenant, adicionar tabela orgs + org_id + RLS por org.
-- ============================================================
...
create table public.afiliados (
  ...
  pct_padrao numeric(5,2) not null default 0,   -- % de comissão
...
create table public.planos (
  ...
  nome text not null,
  recorrencia text not null default 'unica',    -- unica | mensal | anual
...
create table public.matriculas (
  ...
  valor_liquido numeric(12,2) not null,          -- valor após taxa da forma de pgto
  data date not null default current_date,
  status_pagamento status_pagamento not null default 'pago',
  origem text not null default 'manual',         -- manual | hotmart | kiwify | eduzz | stripe
  gateway_id text not null default '',           -- id da transação no gateway (webhook)
  criado_em timestamptz not null default now()
);
...
-- ---------- View de apoio (BI / conferência rápida) ----------
...
-- ---------- Segurança (RLS) ----------
-- MVP: qualquer usuário autenticado lê; só dono/gestor escreve.
-- Fase seguinte: escopo por afiliado (afiliado vê só o próprio braço).

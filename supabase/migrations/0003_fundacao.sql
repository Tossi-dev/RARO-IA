-- ============================================================
-- 0003 — P0 Fundação (Blueprint v3 §6)
-- Venda estendida (braço/gateway/bruto/taxa/liberação/UTM),
-- afiliados com meta/contato/PIX, despesas atribuíveis,
-- metas generalizadas, eventos de webhook e snapshot diário de KPI.
-- Rode APÓS 0001_schema.sql e 0002_expansao.sql.
-- ============================================================
...
create table if not exists public.webhook_eventos (
  ...
  payload jsonb, -- corpo bruto recebido (auditoria/reprocessamento)
...
-- ---------- snapshot diário de KPI (MTD/YoY/pace sem recomputar) ----------
...
-- ---------- RLS (mesmo padrão de 0002: leitura autenticada, escrita dono/gestor) ----------
do $$
declare t text;
begin
...
-- webhook_eventos também recebe INSERT do service role (webhook server-side).
-- O service role ignora RLS por padrão; nada extra a fazer.

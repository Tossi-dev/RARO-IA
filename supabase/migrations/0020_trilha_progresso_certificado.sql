...
-- ============================================================
--
-- Numerada 0020, e não 0018 como o plano da Fase 2 pedia, pelo mesmo motivo
-- registrado no cabeçalho de 0019: os números baixos já foram gastos.
--
-- CONVENÇÃO DE ORDEM (herdada de 0019): em toda política, o escopo do
-- inquilino (`workspace_id = workspace_atual()`) é a PRIMEIRA condição. Não é
-- estética — é o que permite conferir dez políticas de relance, e o que
-- permite ao teste conferir por posição em vez de procurar a string em
-- qualquer lugar do texto, onde ela também aparece dentro de subconsulta.
--
-- ============================================================
-- O PONTO INTEIRO DESTA MIGRAÇÃO: `trilha_marcar_aula`
-- ============================================================
--
-- O mentorado precisa marcar uma aula como concluída. A forma óbvia seria uma
-- política de UPDATE em `progresso_trilha` para ele. Esse caminho já foi
-- tentado neste projeto, em 0012, e uma auditoria contra um Postgres de
-- verdade provou que ele não fecha:
--
--   RLS decide se a LINHA aparece, nunca QUE COLUNA pode ser escrita.
--
-- Com uma política de UPDATE de linha inteira, um PATCH direto no PostgREST
-- (com a anon key, que é pública) permitia forjar `concluida_em = '2020-01-01'`
-- e até mover a linha para outro `mentorado_id`. Foi por isso que 0013 trocou
-- a política pela função `portal_marcar_tarefa`. Esta migração repete aquele
-- desenho, viga por viga — e por isso **não existe política de UPDATE de
-- `progresso_trilha` para mentorado**. A função é o único caminho.
--
-- `security definer` significa que a função roda com os direitos de quem a
-- CRIOU, ignorando a RLS. Isso não é um atalho: é o que torna o `where` dela a
-- linha inteira de defesa. Cada condição fecha uma porta, e nenhuma pode sair.
--
-- UMA DIFERENÇA EM RELAÇÃO A 0013, e ela importa: lá a linha da tarefa sempre
-- existia (o mentor a criou), então bastava um UPDATE. Aqui a linha de
-- progresso NASCE no primeiro clique — o mentorado marca uma aula que nunca
-- teve registro. Por isso é um INSERT ... ON CONFLICT DO UPDATE, e por isso o
-- `unique (mentorado_id, trilha_aula_id)` não é enfeite: é ele que faz o
-- segundo clique atualizar em vez de criar uma segunda linha de progresso para
-- a mesma aula.
...
create table if not exists public.trilha_matricula (
  ...
  -- Uma pessoa não se matricula duas vezes na mesma trilha. Sem isto, duas
  -- linhas com `inicio` diferentes fariam a liberação gradual (tarefa 24)
  -- calcular sobre qual das duas? Não haveria resposta certa.
...
create table if not exists public.progresso_trilha (
  ...
  -- Nulo enquanto aberta, e volta a nulo quando reaberta. Quem escreve este
  -- campo é a função, com `now()` do lado do servidor — nunca um timestamp
  -- que tenha vindo do cliente.
...
create table if not exists public.certificado (
  ...
  mentorado_id uuid not null references public.mentorado (id) on delete cascade,
  trilha_id uuid not null references public.trilha (id) on delete cascade,
  -- Único no banco INTEIRO, não por workspace: o código é o que alguém digita
  -- numa página de verificação pública para conferir se um certificado é real.
  -- Dois certificados com o mesmo código tornariam a verificação ambígua
  -- exatamente no momento em que ela precisa ser categórica.
  codigo text not null unique,
  emitido_em timestamptz not null default now(),
  criado_em timestamptz not null default now(),
  -- Uma trilha concluída gera UM certificado por pessoa. Reemitir substitui,
  -- não acumula.
  unique (mentorado_id, trilha_id)
);
...
-- ---------- progresso_trilha ----------
--
-- LEITURA para o mentorado, sim. ESCRITA, não — nem insert, nem update. O
-- único caminho é `trilha_marcar_aula`, abaixo. Se algum dia alguém
-- acrescentar aqui uma política de update para 'mentorado', terá reaberto,
-- palavra por palavra, o buraco que 0013 fechou.
...
-- ============================================================
-- A função — o único caminho do mentorado até o próprio progresso
-- ============================================================

...
import { codigoValido, destinoDoNegocioValido, gerarCodigo } from "./link";
...
describe("destinoDoNegocioValido", () => {
  it("aceita apenas http/https dos domínios do negócio", () => {
    expect(destinoDoNegocioValido("https://raro-ia.vercel.app/oferta")).toBe(true);
    expect(destinoDoNegocioValido("http://raro-ia.vercel.app/oferta")).toBe(true);
    expect(destinoDoNegocioValido("https://phishing.example/oferta")).toBe(false);
    expect(destinoDoNegocioValido("javascript:alert(1)")).toBe(false);
  });
});
